    ...
    for epoch in range(opt.epochs):
        ...
        if epoch / 10 == 9:
            model.save_pretrained(
                save_directory=opt.save_path + "/center-lr{}-epoch{}".format(opt.learning_rate, epoch))
            text2sql_tokenizer.save_pretrained(
                save_directory=opt.save_path + "/center-lr{}-epoch{}".format(opt.learning_rate, epoch))
        print("epoch", epoch, "finished, loss:", total_loss)
